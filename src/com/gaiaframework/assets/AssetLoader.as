...
package com.gaiaframework.assets
{
	...
	public class AssetLoader extends EventDispatcher implements IAssetLoader
	{
		...
		public function set preloaderInstance(val:IPreloader):void {
			preloader.preloaderInstance = val;
		}
		...
		public function destroy():void {
			disposeAll();
			preloader.destroy();
			busyTimer.removeEventListener(TimerEvent.TIMER, onCheckBusy);
			removeEventListener(BranchLoaderEvent.START, preloader.onStart);
			removeEventListener(AssetEvent.ASSET_PROGRESS, preloader.onProgress);
			removeEventListener(Event.COMPLETE, preloader.onComplete);
		}
		...
		// -- IAssetLoader public methods added
		public function loadAssets(...args):void {
			_loadOnDemand = true;
			var len:int = args.length;
			for (var i:int = 0; i < len; i++) {
				var asset:AbstractAsset = args[i];
				if (asset.percentLoaded < 1)
				{		
					if (!assets[asset])
					{
		...
		public function abortAll():void {
			for each (var i:* in assets) {
				var absAsset:AbstractAsset = i;
				removeAssetListeners(absAsset);
			}
			assetQueue = [];
		...
		private function loadNextAsset():void
		{
			...
			{
				loadingAssets++;
				(assetQueue.shift() as AbstractAsset).loadOnDemand();
			}
		...
		private function reset():void
		{
			assets = new Dictionary();
			completeAssets = totalAssets = loadingAssets = 0;
			if (!SiteController.busy || _loadOnDemand) 
			{
				busyTimer.reset();
		...
		{
			trace("error asset:" + event.asset.id);
